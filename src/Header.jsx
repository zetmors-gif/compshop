    ...
    >
      <div className="max-w-screen-2xl mx-auto px-6 md:px-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6 lg:gap-10">
          <nav className="hidden md:flex items-center gap-8 font-black uppercase italic text-[11px] tracking-[0.3em]">
            {NAVLINKS.map((link) => (
              <a 
                key={link.name} 
                href={link.href}
...
const HUDStatus = () => (
  <div className="hidden lg:flex items-center gap-8 pointer-events-none select-none">
    ...
    <div className="flex items-center gap-4 text-[9px] font-mono text-zinc-600 uppercase font-bold">
       <span className="flex items-center gap-1"><Activity size={12}/> 1ms</span>
       <span className="flex items-center gap-1"><Cpu size={12}/> 14%</span>
    </div>
  </div>
);
...
const JoinButton = ({ className = "" }) => (
  <button className={`items-center gap-2 bg-red-600 text-black px-6 py-2 font-black text-[10px] uppercase tracking-widest hover:bg-white hover:-translate-y-0.5 transition-all active:translate-y-0 skew-x-[-15deg] ${className}`}>
