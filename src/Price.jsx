...
export function Price() {
  return (
    <section id='price' className="w-full bg-[#030303] py-10 px-4 md:px-10 border-t border-white/5 relative overflow-hidden">
      {/* Background Decor */}
            ...
            >
              ...
              <div className="flex justify-between items-center mb-10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[14px] font-mono text-zinc-500 uppercase tracking-widest">{plan.id}</span>
                </div>
                <div className={`text-[14px] font-black px-3 py-1 ${plan.active ? 'bg-red-600' : 'bg-zinc-800 text-zinc-400'}`}>
                  {plan.status}
                </div>
              </div>

              <h3 className="text-7xl font-sans italic font-bold uppercase tracking mb-8 group-hover:text-red-600 transition-colors">
                {plan.name}
              </h3>
              ...
              <div className="grid grid-cols-2 gap-4 mb-12">
                {plan.specs.map((spec, idx) => (
                  <div key={idx} className="border border-white/5 p-3 bg-white/2 hover:bg-white/5 transition-colors">
                    <div className="text-[12px] text-zinc-600 font-alumni italic uppercase mb-1 font-bold">Slot 0{idx + 1}</div>
                    <div className="text-[18px] font-alumni italic font-black tracking-wide uppercase">{spec}</div>
                  </div>
                ))}
                <div className="border border-red-600/20 p-3 bg-red-600/3 flex items-center justify-between font-alumni italic">
                   <div className="text-[20px] text-red-600/50 uppercase font-bold tracking-tighter leading-none">Load<br/>Level</div>
                   <div className="text-[25px] font-black text-red-600 italic leading-none">{plan.load}</div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-baseline gap-2">
                  <span className="text-6xl font-bold tracking-normal italic">{plan.price}</span>
                  <span className="text-red-600 font-bold font-alumni text-xl uppercase tracking-widest italic">rub</span>
                  <span className="text-zinc-600 text-xl font-alumni ml-auto tracking-widest">/HOUR</span>
                </div>
                ...
                <button className="relative w-full overflow-hidden group/btn block outline-none">
                  ...
                  <div className="relative z-10 py-4 border border-white/10 group-hover/btn:border-red-600 transition-colors flex items-center justify-center gap-3">
                    <span className="text-[18px] font-black uppercase group-hover/btn:text-black transition-colors">Инициация входа</span>
